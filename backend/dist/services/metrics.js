...
exports.getAccuracyMetrics = exports.getTeamMetrics = exports.getReviewVelocity = exports.recordReviewMetrics = void 0;
const db_1 = __importDefault(require("../db"));
/**
 * Record review metrics
 */
const recordReviewMetrics = async (data) => {
    ...
    try {
        ...
        if (!data.repositoryId) {
            const existing = await db_1.default.reviewMetrics.findFirst({
                where: {
                    repositoryId: null,
                    date: today,
                },
            ...
            if (existing) {
                await db_1.default.reviewMetrics.update({
                ...
                await db_1.default.reviewMetrics.create({
                    data: {
                        repositoryId: null,
                        date: today,
        ...
        await db_1.default.reviewMetrics.upsert({
            where: {
                repositoryId_date: {
...
exports.recordReviewMetrics = recordReviewMetrics;
/**
 * Get review velocity (reviews per day/week/month)
 */
const getReviewVelocity = async (options) => {
    const metrics = await db_1.default.reviewMetrics.findMany({
        where: {
...
exports.getReviewVelocity = getReviewVelocity;
/**
 * Get team performance metrics
 */
const getTeamMetrics = async (options) => {
    const metrics = await db_1.default.reviewMetrics.findMany({
        where: {
            repositoryId: options.repositoryId || undefined,
            date: {
    ...
    return {
        ...
        fixAcceptanceRate: totals.fixesApplied + totals.fixesRejected > 0
            ? (totals.fixesApplied / (totals.fixesApplied + totals.fixesRejected)) * 100
            : 0,
        criticalIssueRate: totals.totalIssues > 0 ? (totals.criticalIssues / totals.totalIssues) * 100 : 0,
    };
};
 ...
 */
const getAccuracyMetrics = async (options) => {
    const where = {
