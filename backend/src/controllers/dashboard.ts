 ...
 */
export const getDashboardStatsData = async (userId?: string) => {
    ...
    const completedReviewsCount = await prisma.review.count({
        where: {
            status: 'COMPLETED',
            pullRequest: repoFilter,
        },
    ...
    const prsWithLatestReviews = await prisma.pullRequest.findMany({
        where: userId ? { repository: { userId } } : {},
        include: {
            reviews: {
                where: {
    ...
    for (const pr of prsWithLatestReviews) {
        ...
        if (latestCompletedReview) {
            ...
            const criticalCount = latestCompletedReview.issues.filter(
                (issue: any) => issue.severity === 'critical' || issue.severity === 'security'
            ).length;
            criticalIssuesCount += criticalCount;

            // Count all issues by severity from this PR's latest review
            latestCompletedReview.issues.forEach((issue: any) => {
    ...
    const completedReviewsWithTiming = await prisma.review.findMany({
        where: {
            status: 'COMPLETED',
            pullRequest: repoFilter,
        },
    ...
    if (completedReviewsWithTiming.length > 0) {
        const totalTime = completedReviewsWithTiming.reduce((sum: number, review: any) => {
    ...
    const formattedRecentPRs = (recentPRs as any[]).map((pr) => {
        const latestReview = pr.reviews[0];
...
async function getWeeklyReviewTrend(userId?: string) {
    try {
        // Build repository filter for user's repos
        const repoFilter = userId ? { repository: { userId } } : {};
        ...
        // Group by date
        const trend: Record<string, { reviews: number; approved: number; rejected: number; issues: number }> = {};
        ...
        for (const review of reviews as any[]) {
            ...
            if (trend[key]) {
                trend[key].reviews++;
                if (review.status === 'COMPLETED') trend[key].approved++;
                if (review.status === 'FAILED') trend[key].rejected++;
