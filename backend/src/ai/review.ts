...
import { buildEnhancedReviewPrompt } from './enhancedPrompt';
import { ReviewResponseSchema, ReviewIssue } from './schema';
import { detectLanguageFromFile } from '../utils/language';
import { getRulesForLanguage } from './reviewRules';
...
const countLinesInPatch = (patch: string): { added: number; removed: number; total: number } => {
    if (!patch) return { added: 0, removed: 0, total: 0 };
...
const CWE_BY_CATEGORY: Record<string, string> = {
    'security': 'CWE-937',
    'critical': 'CWE-937',
...
export const reviewPullRequest = async (
    ...
    const fileReviews = await Promise.all(filesToReview.map(async (file, fileIndex) => {
        try {
            ...
            const language = detectLanguageFromFile(file.filename);
            ...
            if (file.patch.length > MAX_CHUNK_SIZE) {
                ...
                let match;
                const hunkMatches: Array<{ index: number }> = [];
                ...
                if (hunkMatches.length > 0) {
                    ...
                    for (let i = 0; i < hunkMatches.length; i++) {
                        ...
                        if (currentChunk.length + hunkContent.length <= MAX_CHUNK_SIZE) {
                            ...
                            if (currentChunk) patches.push(currentChunk);
                            currentChunk = hunkContent;
                        }
                    }
                    if (currentChunk) patches.push(currentChunk);
            ...
            const chunkReviews = await Promise.all(patches.map(async (patchChunk, index) => {
                ...
                const prompt = buildEnhancedReviewPrompt(patchChunk, language, reviewRules, {
                ...
                try {
                    ...
                    if (useClaude) {
                        ...
                        content = await openaiChat(
                            ...
                            { temperature: 0.1, format: 'json' } // GPT-4: 0.1 for accuracy
                        );
                    } else if (useGemini) {
            ...
            // Merge Chunk Results
            const validReviews = chunkReviews.filter((r): r is NonNullable<typeof r> => r !== null);
  ...
  "issues": [
    {
      ...
      "category": "code-quality"
                    ...
                    if (fallbackContent) {
                        ...
                        if (cleanFallback.startsWith('```')) cleanFallback = cleanFallback.slice(3);
                        ...
                        if (jsonMatch) cleanFallback = jsonMatch[0];
                        
                        console.log(`[AI] Parsing fallback JSON...`);
                        const fallbackParsed = JSON.parse(cleanFallback);
                        const fallbackValidated = ReviewResponseSchema.parse(fallbackParsed);
                ...
                return {
                    summary: summaryText,
                    riskLevel: basicIssues.length > 0 ? 'MEDIUM' as const : 'LOW' as const,
    ...
    const REVERSE_RISK_MAP = ['LOW', 'MEDIUM', 'HIGH'] as const;
...
`.trim();
    ...
    try {
        const breakingChanges = await detectBreakingChanges(filesToReview);
        if (breakingChanges.length > 0) {
            breakingChangesReport = '\n\n' + generateBreakingChangeReport(breakingChanges);
            ...
            for (const bc of breakingChanges) {
                allIssues.push({
                    ...
                    language: detectLanguageFromFile(bc.file),
                });
            }
        }
    } catch (error: any) {
